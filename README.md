[README.md](https://github.com/user-attachments/files/28039018/README.md)
<div align="center">

# Olá, eu sou o Rower Bomfim! <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30px"/>

### 🚀 Desenvolvedor Fullstack em Formação | Java · Web · Cloud AWS

<p>
  Estudante de <strong>Análise e Desenvolvimento de Sistemas (ADS)</strong> em transição de carreira para tecnologia.<br/>
  Apaixonado por construir softwares eficientes, escaláveis e bem estruturados — com foco em <strong>Backend Java</strong> e <strong>Cloud Computing (AWS)</strong>.
</p>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rower-bomfim-77966b178/)
[![Gmail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:rower.bomfim@hotmail.com)

</div>

---

## 🛠️ Tecnologias & Ferramentas

<div align="center">

### Backend
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white)

### Frontend
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

### Banco de Dados
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)

### DevOps & Cloud
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

</div>

---

## 📚 Aprendizado Atual

| Status | Tópico |
|--------|--------|
| 🔄 Em progresso | APIs RESTful com **Spring Boot** |
| 🔄 Em progresso | Integração Java + MySQL via **JDBC / JPA / Hibernate** |
| 🔄 Em progresso | Certificação **AWS Certified Cloud Practitioner** |

---

## 🎓 Certificações

<details>
<summary>☕ <strong>Backend — Java</strong></summary>
<br/>

- **Java: Criando a sua primeira aplicação** — Fundamentos e sintaxe · *Alura*
- **Java: Aplicando a Orientação a Objetos** — Abstração, herança, encapsulamento e polimorfismo · *Alura*
- **Java: Trabalhando com Listas e Coleções de Dados** — Estruturas de dados · *Alura*

</details>

<details>
<summary>🌐 <strong>Frontend & Lógica — JavaScript / Web</strong></summary>
<br/>

- **Lógica de Programação: Mergulhe em programação com JavaScript** · *Alura*
- **Lógica de Programação: Exploração de Funções e Listas** · *Alura*
- **Lógica de Programação: Prática com Desafios** · *Alura*
- **HTML e CSS: Ambientes de desenvolvimento, Variáveis e Flexbox** · *Alura*
- **HTML e CSS: Responsividade e Publicação de Projetos** · *Alura*

</details>

<details>
<summary>🗄️ <strong>Banco de Dados & IA</strong></summary>
<br/>

- **MySQL: Curso Completo de Bancos de Dados Relacionais** — 40h · *Curso em Vídeo*
- **Copilotos Inteligentes no Apoio à Construção de Soluções** · *DIO*

</details>

---

## 📊 GitHub Stats

<div align="center">

![Rower's GitHub Stats](https://github-readme-stats.vercel.app/api?username=RowerBomfim&show_icons=true&theme=tokyonight&hide_border=true&locale=pt-br)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=RowerBomfim&layout=compact&theme=tokyonight&hide_border=true&locale=pt-br)

</div>

---

<div align="center">

*"A jornada de mil milhas começa com um único passo."* 🌱

**Obrigado por visitar meu perfil! Bora conectar?** 👇

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rower-bomfim-77966b178/)

</div>
